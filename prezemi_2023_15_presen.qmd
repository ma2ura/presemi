...
となる$\lambda$と$\boldsymbol{v}$が存在する。
...
df_senbei <- readr::read_table("data/senbei.txt")
...
これを1から0の間の値を取るように変換したものがKMO値である。
...
`KMO()`の引数には、データフレームがはいるため，データフレームには必要な変数のみを入れておくようにする。
...
観測変数間の相関行列が求まりました。これを$\Sigma$(シグマ)とする。
...
g <- g + geom_text_repel(
  ...
  colour=ifelse(rownames(f1_scores)=="Japan(14)", "magenta",
         ifelse(rownames(f1_scores)=="Japan(18)", "magenta",
         ifelse(rownames(f1_scores)=="Germany(14)", "magenta",
         ifelse(rownames(f1_scores)=="France(18)", "magenta", "blue4"
         )))),
  alpha=0.9,size=3
)
print(g)
